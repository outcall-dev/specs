...
| S007-EC-008 | Port 53 already in use | `outcalld` logs an error and aborts daemon startup. It **MUST NOT** continue with managed containers while DNS enforcement is unavailable. |
...
| S007-EC-015 | DNS amplification or unmanaged source | The DNS server binds only to the bridge gateway IP and resolves every source through the managed-container identity index. Unknown sources receive REFUSED and are counted as blocked. |
| S007-EC-016 | Cached answer contains a newly disallowed private address | Reapply the current matched rule's address policy; do not trust the policy that was active when the raw answer was cached. |
| S007-EC-017 | Direct-IP answer uses a different address family than the managed source | Return the allowed DNS answer, but do not create a mixed-family nftables grant. The bridge's family policy remains authoritative. |
