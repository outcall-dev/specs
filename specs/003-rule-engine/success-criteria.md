...
| S003-SC-004 | Given equal-priority rules in `00-base.yaml`, `25-team.yaml`, and `50-custom.yaml`, rules from `00-base.yaml` are evaluated first. |
...
| S003-SC-008 | `action: enrich` and `enrich` configuration are rejected without executing project code in the privileged daemon. |
...
| S003-SC-011 | Rules with `log: true` produce structured audit entries containing rule ID, source file, decision, and tracing timestamp without logging headers, bodies, or environment values. |
